import source_classify.config as config
import source_classify.utils as utils
import source_classify.data_formatting.aptos2019 as aptos2019
...
def correct_file_names(original_all_folders_path, annot_csv, training_all_folder_path, clear_folder=True, img_name_prefix=""):
    ...
    with open(train_csv_path, mode='r') as csv_file:
        ...
        for i, row in enumerate(csv_reader):
            train_labels[row["Image name"]] = row["Retinopathy grade"]
            if FULL_LABELS[int(row["Retinopathy grade"])] not in unique_labels:
                unique_labels.append(FULL_LABELS[int(row["Retinopathy grade"])])
...
if __name__ == "__main__":
    correct_file_names("/mnt/nvme0n1p3/MySSD/Programming/AI/ClientProjects/develop/FundusImages/data/org_data/IDRiD/B.%20Disease%20Grading/B. Disease Grading/1. Original Images/a. Training Set/",
                       "/mnt/nvme0n1p3/MySSD/Programming/AI/ClientProjects/develop/FundusImages/data/org_data/IDRiD/B.%20Disease%20Grading/B. Disease Grading/2. Groundtruths/a. IDRiD_Disease Grading_Training Labels.csv",
                       config.DATA_FOLDERS["training_data"] + config.IDRID + os.sep,
                       img_name_prefix="train_")

    correct_file_names("/mnt/nvme0n1p3/MySSD/Programming/AI/ClientProjects/develop/FundusImages/data/org_data/IDRiD/B.%20Disease%20Grading/B. Disease Grading/1. Original Images/b. Testing Set/",
                        "/mnt/nvme0n1p3/MySSD/Programming/AI/ClientProjects/develop/FundusImages/data/org_data/IDRiD/B.%20Disease%20Grading/B. Disease Grading/2. Groundtruths/b. IDRiD_Disease Grading_Testing Labels.csv",
                        config.DATA_FOLDERS["training_data"] + config.IDRID + os.sep,
                        img_name_prefix="test_",
                        clear_folder=False)

    aptos2019.split_train_test(config.DATA_FOLDERS["training_data"] + config.IDRID + os.sep + "all" + os.sep,
                               split_per=0.5)